...
1. Actually connect Gmail as a real user in the browser to confirm the end-to-end consent flow works (only a human can do this step).
2. `RawEmail` model + BullMQ worker + Claude-based extraction pipeline for auto-populating applications from email — will call `createApplication`/`updateApplication` from `jobApplication.service.ts` directly (in-process, no HTTP hop), and will need to decrypt the stored refresh token (`src/lib/crypto.ts`'s `decrypt`) to call the Gmail API on the user's behalf.
